...
def build_vectorstore():
    ...
    print(f"📁 Guardando vectorstore en: {output_dir}")

    # Si el archivo no existe en la ruta indicada, intentar búsqueda automática
    if not os.path.exists(dataset_path):
        print(f"⚠️ Dataset no encontrado en '{dataset_path}'. Buscando archivos .xlsx/.xls en el proyecto...")
        found = None
        ...
        if found:
            dataset_path = found
            print(f"🔎 Usando dataset encontrado: {dataset_path}")
        else:
